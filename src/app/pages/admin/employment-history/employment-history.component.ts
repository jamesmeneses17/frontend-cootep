...
export class EmploymentHistoryComponent implements OnInit {
  ...
  loadHistories(): void {
    this.http.get<any[]>('http://localhost:3000/employment-history/admin/all').subscribe({
      next: data => {
        this.histories = data;
        this.applyFilters();
      },
  ...
  loadContracts(): void {
    this.http.get<any[]>('http://localhost:3000/contract-type').subscribe({
  ...
  deleteHistory(id: number): void {
    if (confirm('¿Estás seguro de que deseas eliminar este historial laboral?')) {
      this.http.delete(`http://localhost:3000/employment-history/${id}`).subscribe({
        next: () => {
